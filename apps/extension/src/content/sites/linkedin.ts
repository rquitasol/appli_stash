...
class LinkedInExtractor implements JobSiteExtractor {
  ...
  private extractFromJobCard(): JobData | null {
    try {
      ...
      if (!jobDetailsWrapper) {
        console.log(
          "AppliStash: No job details wrapper/container found"
        );
      ...
      return this.extractDataFromJobDetailsWrapper(
        jobDetailsWrapper as HTMLElement
      );
    } catch (error) {
      console.error(
   ...
   */
  private extractDataFromJobDetailsWrapper(
    wrapper: HTMLElement
  ): JobData | null {
    try {
      ...
      console.log(
        "AppliStash: Found LinkedIn job details wrapper elements",
        {
          title: titleElement?.textContent,
          company: companyElement?.textContent,
          location: locationElement?.textContent,
      ...
      if (!title || !company) {
        console.log(
          "AppliStash: Missing required job data from details wrapper",
          { title, company }
        );
        return null;
      }
      ...
      console.error(
        "AppliStash: Error extracting from job details wrapper",
        error
      );
      return null;
    }

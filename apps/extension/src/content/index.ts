...
function checkAuth(): Promise<boolean> {
  return new Promise((resolve) => {
    chrome.storage.local.get(['user'], (result) => {
      if (result.user) {
        console.log('AppliStash: User is authenticated', result.user);
        isAuthenticated = true;
        currentUser = result.user;
        resolve(true);
      } else {
        console.log('AppliStash: User is NOT authenticated');
...
async function handleOverlayInjection() {
  console.log('AppliStash: Handling overlay injection for URL:', window.location.href);

  // First check if user is authenticated
  const authenticated = await checkAuth();
  if (!authenticated) {
    console.log('AppliStash: Authentication required. Not loading overlay.');
...
checkAuth().then(() => {
  if (isAuthenticated) {
    ...
    console.log('AppliStash: Not authenticated, skipping initial injection');
  }
});
...
function setupJobCardClickListeners() {
  if (window.location.href.includes('linkedin.com/jobs')) {
    console.log('AppliStash: Setting up job card click listeners');
    ...
    document.addEventListener('click', (event) => {
      ...
      const jobCard = target.closest('.job-card-container, .jobs-search-results__list-item');
      ...
      if (jobCard) {
        ...
        setTimeout(() => {
          if (currentUser) {
            ...
            const existingOverlays = document.querySelectorAll('.appli-stash-overlay');
    ...
    console.log('AppliStash: Job card click listeners setup complete');
  }
}

// Also wait for the DOM to be fully loaded
window.addEventListener('load', () => {
  console.log('AppliStash: Page loaded, checking if we should inject overlay again');
  if (isAuthenticated) {
    handleOverlayInjection();
    ...
    console.log('AppliStash: Not authenticated, skipping load injection');
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'showOverlay') {
    if (!isAuthenticated) {
      console.log('AppliStash: Cannot show overlay - not authenticated');
      sendResponse({
        success: false,
...
const urlObserver = new MutationObserver(() => {
  ...
  if (currentUrl !== lastUrl) {
    ...
    if (isAuthenticated) {
      ...
      console.log('AppliStash: Not authenticated, skipping URL change injection');
    }
  }
});
...
function setupJobSelectionObserver() {
  ...
  console.log('AppliStash: Setting up job selection observer');
  ...
  const jobSelectionObserver = new MutationObserver(() => {
    ...
    const currentJobTitle = titleElement?.textContent?.trim() || '';
    const currentCompanyName = companyElement?.textContent?.trim() || '';

    // Check if the job has changed
    if (
      (currentJobTitle && currentJobTitle !== lastJobTitle) ||
      (currentCompanyName && currentCompanyName !== lastCompanyName)
    ) {
...
if (isAuthenticated) {
  ...
  console.log('AppliStash: Started mutation observer for URL changes');

  // Also set up the job selection observer
  setupJobSelectionObserver();
} else {
  console.log('AppliStash: Not starting observers - not authenticated');
}

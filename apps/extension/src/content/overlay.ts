...
import { ApplicationPriority, ApplicationStatus } from '@applistash/shared';
...
export function injectOverlay(user: User): void {
  ...
  if (!jobData) {
    console.error('AppliStash: Could not extract job data from page');
    return;
  }
  ...
  const overlayElement = createOverlayUI(jobData, user, (applicationData: JobData) => {
  ...
  console.log('AppliStash: Injecting overlay into document.body');
 ...
 */
async function saveApplication(jobData: JobData, user: User): Promise<void> {
  try {
    ...
    if (user.token) {
      ...
      console.log('AppliStash: Adding JWT token to request');
    } else {
    ...
    console.log('AppliStash: Successfully saved application', data);

    // Show a success message
    showNotification('Success', 'Application saved to AppliStash!');
  } catch (error) {
    console.error('AppliStash: Error saving application', error);
...
function showNotification(title: string, message: string): void {
  ...
  notification.style.backgroundColor = title === 'Success' ? '#4CAF50' : '#F44336';
